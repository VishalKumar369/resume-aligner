...
# Selectable typography presets. Every layout stays single-column and
# table-free (what ATS parsers read reliably); they differ only in colour,
# name size, and density — enough to look distinct and professional. "classic"
# is the plain default and the base the others are derived from.
_ACCENT = "#1F3A5F"  # a professional navy for headings/name


def _modern_styles() -> Dict[BlockKind, BlockStyle]:
    styles = dict(STYLE)
    styles[BlockKind.NAME] = replace(STYLE[BlockKind.NAME], size=19, color=_ACCENT)
    styles[BlockKind.HEADING] = replace(STYLE[BlockKind.HEADING], color=_ACCENT)
    styles[BlockKind.SUBHEADING] = replace(STYLE[BlockKind.SUBHEADING], color="#222222")
    return styles


def _compact_styles() -> Dict[BlockKind, BlockStyle]:
    return {
        kind: replace(
            style,
...
LAYOUTS: Dict[str, Dict[BlockKind, BlockStyle]] = {
    "classic": dict(STYLE),
    "modern": _modern_styles(),
    "compact": _compact_styles(),
}
...
def _layout_styles(layout: Optional[str]) -> Dict[BlockKind, BlockStyle]:
    return LAYOUTS.get((layout or "classic").lower(), LAYOUTS["classic"])
...
def render_pdf_with_page_count(
    ...
    layout_styles = _layout_styles(layout)

    def _para_style(kind: BlockKind) -> ParagraphStyle:
        spec = layout_styles.get(kind, layout_styles[BlockKind.PARAGRAPH])
    ...
    styles = {kind: _para_style(kind) for kind in layout_styles}
